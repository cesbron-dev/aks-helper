...
`aks-helper` manages AKS kubeconfigs under `~/.kube/aks` and lets you run
commands against a chosen cluster. As an agent you have **no TTY**, so never use
the interactive fuzzy finder — always use the flag-driven, non-interactive paths
described here.
...
2. **Import a cluster** only if the one you need is not listed. This requires a
   ...
   `sync` defaults to `--login azurecli`, which reuses the existing `az`
   session and never prompts. Do **not** switch to `devicecode`/`interactive`
   login modes — they block waiting for a human.
...
- Resolve a cluster's kubeconfig path with `aks-helper path <name>` when a tool
  needs an explicit `--kubeconfig` flag (e.g. tools that don't read the env).
